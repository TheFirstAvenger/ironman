defmodule Ironman.Checks.GitHooksConfigTest do
  ...
  describe "run" do
    ...
    test "runs when config not present - nothing else present" do
      ...
      assert String.starts_with?(Config.get(config2, :config_dev_exs), "import Config\n\nconfig :git_hooks,")
    ...
    test "runs when config not present - *.exs present" do
      config =
        ...
        |> Config.set(:config_test_exs, "import Config\n\n")
        |> Config.set(:config_prod_exs, "import Config\n\n")
